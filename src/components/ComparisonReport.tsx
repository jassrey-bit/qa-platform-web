    ...
    <div className="flex flex-col gap-gutter-lg">
      ...
      {visual && (
        <section className="rounded-2xl border border-outline-variant/20 bg-surface-container/60 p-6">
          ...
          {visual.available ? (
            <>
              <p className="mt-1 text-sm text-on-surface-variant">Estado: {visual.status}</p>
              <pre className="mt-2 whitespace-pre-wrap rounded-xl border border-outline-variant/20 bg-surface-container-low p-4 font-code text-xs text-on-surface">
                {visual.findings}
              </pre>
            </>
          ) : (
